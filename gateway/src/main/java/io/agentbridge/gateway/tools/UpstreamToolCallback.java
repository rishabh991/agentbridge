...
public class UpstreamToolCallback implements ToolCallback {
    ...
    public String call(String toolInput) {
        ...
        try {
            ...
            audit.record(tool.name(), tool.upstream(), null, AuditEvent.OUTCOME_GATEWAY_ERROR, null, 0L, null);
        ...
        try {
            ...
            audit.record(tool.name(), tool.upstream(), arguments, AuditEvent.OUTCOME_OK,
                    response.getStatusCode().value(), millis, idempotencyKey);
            return response.getBody() == null ? "{}" : response.getBody();

        } catch (RestClientResponseException e) {
            var millis = elapsedMillis(started);
            audit.record(tool.name(), tool.upstream(), arguments, AuditEvent.OUTCOME_UPSTREAM_ERROR,
            ...
            audit.record(tool.name(), tool.upstream(), arguments, AuditEvent.OUTCOME_GATEWAY_ERROR,
                    null, millis, idempotencyKey);
